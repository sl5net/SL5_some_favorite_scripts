#!/bin/bash
# Skriptname: setup_usb_nfs_mount.sh
# Zweck: Richtet den NFS-Automount für den USB-Stick des ix4-300d NAS ein.
# Ziel-Mountpunkt: /mnt/ix4_usb_nfs

echo "=== Automatisches Setup-Skript für NFS-Mount des USB-Sticks: /mnt/ix4_usb_nfs ==="
echo ""

# ---- Konfiguration ----
LOCAL_MOUNT_POINT="/mnt/ix4_usb_nfs"
NAS_IP="[YourNASip]"
NFS_EXPORT_PATH_USB="/nfs/usb" # Gemäß deiner früheren 'showmount -e' Ausgabe

# Dateinamen für die systemd-Units
MOUNT_UNIT_FILENAME="mnt-ix4_usb_nfs.mount"
AUTOMOUNT_UNIT_FILENAME="mnt-ix4_usb_nfs.automount"
...
UNIT_NAME_FOR_COMMANDS="mnt-ix4_usb_nfs"
...
if [ $? -eq 0 ]; then
    echo "-> Mountpunkt '$LOCAL_MOUNT_POINT' existiert oder wurde erstellt."
else
    echo "FEHLER: Konnte Mountpunkt '$LOCAL_MOUNT_POINT' nicht erstellen. Bitte manuell prüfen."
    exit 1
fi
echo ""
...
# Vorher löschen, um sicherzustellen, dass wir eine saubere Datei haben
sudo rm -f "$MOUNT_UNIT_FILE_PATH"
sudo tee "$MOUNT_UNIT_FILE_PATH" > /dev/null <<EOF
[Unit]
Description=Mount ix4-300d USB (NFS) to ${LOCAL_MOUNT_POINT}
Requires=network-online.target
After=network-online.target

[Mount]
What=${NAS_IP}:${NFS_EXPORT_PATH_USB}
...
# Vorher löschen für sauberen Zustand
sudo rm -f "$AUTOMOUNT_UNIT_FILE_PATH"
sudo tee "$AUTOMOUNT_UNIT_FILE_PATH" > /dev/null <<EOF
[Unit]
Description=Automount ix4-300d USB (NFS) to ${LOCAL_MOUNT_POINT}
Requires=network-online.target
After=network-online.target
...
# Deaktivieren über Dateipfad ist oft robuster, falls Namensauflösungsprobleme bestehen
echo "-> Versuche Deaktivierung über Dateipfad: $AUTOMOUNT_UNIT_FILE_PATH"
sudo systemctl disable "$AUTOMOUNT_UNIT_FILE_PATH" # Fehler hier sind ok, wenn Symlink nicht existiert
...
echo "=== Setup-Skript für USB-NFS (/mnt/ix4_usb_nfs) abgeschlossen ==="
