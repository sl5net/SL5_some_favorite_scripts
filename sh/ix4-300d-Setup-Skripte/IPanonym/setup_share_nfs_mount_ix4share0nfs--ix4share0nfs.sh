...
echo "=== Automatisches Setup-Skript für NFS-Mount: /mnt/ix4share0nfs ==="
echo ""

# ---- Konfiguration ----
LOCAL_MOUNT_POINT="/mnt/ix4share0nfs" # Neuer Pfad mit Unterstrich
NAS_IP="[YourNASip]"
NFS_EXPORT_PATH="/nfs/share"

# Dateinamen für die systemd-Units (abgeleitet vom NEUEN Mountpunkt)
# Systemd konvertiert / zu - und behält oft _ bei oder wandelt es in \x5f um.
# /mnt/ix4share0nfs -> mnt-ix4\x2d300d-share_nfs
# Der tatsächliche Dateiname wird mnt-ix4-300d-share_nfs.mount sein
MOUNT_UNIT_FILENAME="mnt-ix4-300d-share_nfs.mount"
AUTOMOUNT_UNIT_FILENAME="mnt-ix4-300d-share_nfs.automount"
...
# Für /mnt/ix4share0nfs:
# Bindestriche (-) werden zu \x2d
# Unterstriche (_) werden zu \x5f oder bleiben _ (abhängig von systemd-Version/Kontext)
# Wir versuchen es hier mit dem direkten Namen, wie er in der Datei sein wird,
# und verlassen uns auf systemd-escape bei den Befehlen oder verwenden Dateipfade.
# Für die Shell-Kommandos wird systemd 'mnt-ix4\x2d300d-share_nfs' oder 'mnt-ix4\x2d300d-share\x5fnfs' erwarten.
# Wir verwenden hier den robusteren Ansatz über Dateipfade für enable/disable
# und für start/stop/status den Namen, den der Symlink haben wird.
UNIT_NAME_FOR_COMMANDS="mnt-ix4\\x2d300d-share_nfs" # Basisname
...
sudo systemctl disable "$AUTOMOUNT_UNIT_FILE_PATH" # Sollte "no such file or directory" oder Warnung geben, wenn kein Symlink da ist.
...
# Hier verwenden wir den Namen, den systemd aus dem Symlink-Namen ableiten sollte.
# Der Symlink wird /etc/systemd/system/mnt-ix4-300d-share_nfs.automount heißen.
# Der Unit-Name, den systemd daraus macht und den wir hier verwenden sollten, ist
# mnt-ix4\x2d300d-share_nfs.automount (Unterstrich bleibt, Bindestrich wird zu \x2d).
sudo systemctl restart "mnt-ix4\\x2d300d-share_nfs.automount"
if [ $? -eq 0 ]; then
    ...
    sudo systemctl status "mnt-ix4\\x2d300d-share_nfs.automount" --no-pager -n 0
...
echo "-> Status für mnt-ix4\\x2d300d-share_nfs.automount:"
sudo systemctl status "mnt-ix4\\x2d300d-share_nfs.automount" --no-pager
echo ""
...
if [ $ls_exit_code -eq 0 ]; then
    ...
    echo ""
    echo "-> Status für mnt-ix4\\x2d300d-share_nfs.mount (sollte jetzt aktiv sein):"
    sudo systemctl status "mnt-ix4\\x2d300d-share_nfs.mount" --no-pager
else
    echo "FEHLER: Zugriff auf '$LOCAL_MOUNT_POINT' fehlgeschlagen (Exit Code: $ls_exit_code)."
    echo "-> Status für mnt-ix4\\x2d300d-share_nfs.mount (wird wahrscheinlich Fehler zeigen):"
    sudo systemctl status "mnt-ix4\\x2d300d-share_nfs.mount" --no-pager
    echo ""
    echo "-> Journal für mnt-ix4\\x2d300d-share_nfs.mount (letzte 20 Zeilen):"
    sudo journalctl -n 20 -xeu "mnt-ix4\\x2d300d-share_nfs.mount" --no-pager
fi
echo ""
